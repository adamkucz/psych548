...
legend(x = "topleft", # takes keywords OR x, y coordinates
...
The `extrafont` package extends the fonts available for plotting in R:
1. Install the package `install.packages("extrafont")`
2. Import the fonts: `extrafont::font_import()`
...
.small[
R's `chickwts` data comes from a between-subjects experiment on the effect of chicken feed implements on chicken growth rate at 6 weeks old:
...
val <- 1.96
polygon(c(x[x <= -val], -val), c(y[x <= -val], min(y[x <= -val])), col=rgb(0.51, 0.44, 1, alpha = 0.6))
polygon(c(x[x >= val], val), c(y[x >= val], min(y[x >= val])), col=rgb(0.51, 0.44, 1, alpha = 0.6))
```



<!-- Desntiy plots of your own data -->
<!-- ```{r} -->
<!-- set.seed(3) -->
<!-- x <- rnorm(200) -->
<!-- y <- rnorm(150, mean = 1) -->
...
<!-- RESOURCES -->
<!-- Named colors in base R: http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf -->
<!-- https://rich-iannone.github.io/DiagrammeR/ -->
<!-- FONTS: https://www.statmethods.net/advgraphs/parameters.html -->